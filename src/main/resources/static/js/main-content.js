    ...
    async toggleWordFavorite(card) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const favoriteBtn = card.querySelector('.word-This code disables toast notifications by modifying the showToast function.
<replit_final_file>
// 중복 선언 방지
if (typeof window.EnhancedIntegratedLearningManager !== 'undefined') {
    console.log('🔄 기존 EnhancedIntegratedLearningManager 제거');
...
class EnhancedIntegratedLearningManager {
    ...
    async toggleWordFavorite(card) {
        ...
        try {
            ...
            if (response.ok) {
                const result = await response.json();
                const favoriteBtn = card.querySelector('.word-favorite');favoriteBtn) {
